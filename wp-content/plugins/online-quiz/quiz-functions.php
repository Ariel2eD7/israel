...
function display_online_quiz() {
    ...
    <input type='submit' value='הגש מבחן' style='padding: 12px 25px; background-color: #4CAF50; color: white; border: none; border-radius: 5px; font-size: 18px; cursor: pointer; transition: background-color 0.3s ease;'>
        ...
        document.getElementById('quiz-form').addEventListener('submit', function(e) {
            e.preventDefault(); 
            var quizId = new URLSearchParams(window.location.search).get('quiz_id');
            if (!quizId) { alert('Quiz ID is missing!'); return; }
            var formData = new FormData(this), result = 0, correctAnswers = " . json_encode(array_column($questions, 'answer')) . ", userAnswers = [];
            for (var [name, selectedAnswer] of formData.entries()) { userAnswers.push(selectedAnswer); }
            var feedback = '';
            for (var i = 0; i < userAnswers.length; i++) {
                var isCorrect = userAnswers[i] === correctAnswers[i];
                feedback += `<p><strong>${i + 1}. ${json_encode(array_column($questions, 'question'))[i]}</strong><br>Your Answer: ${userAnswers[i]}${isCorrect ? ' (Correct)' : ' (Incorrect)'}<br>Correct Answer: ${correctAnswers[i]}</p>`;
                if (isCorrect) result++;
            }
            var timeSpentInSeconds = 90 * 60 - timeRemaining;
            window.location.href = 'https://indexing.co.il/quiz-results?quiz_id=' + quizId + '&answers=' + encodeURIComponent(JSON.stringify(userAnswers)) + '&score=' + result + '&time_spent=' + timeSpentInSeconds;
        });

        let timerDisplay = document.getElementById('timer'), timeRemaining = 90 * 60;
