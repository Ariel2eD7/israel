...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function populateDropdown(id, items) {
    ...
    container.innerHTML = `<input type="text" class="search-input" oninput="filterOptions(\'${id}\', this.value)" placeholder="Search..." style="background-color: var(--button-bg-color) !important; width: 100%; padding: 8px; margin-bottom: 8px; border: 1px solid #ccc;">` +
      items.map(item => {
        ...
        function clearSubsequentDropdowns(id) {
            ...
            for (let i = start; i < ids.length; i++) {
                ...
                if (box && options) {
                    ...
                    box.style.backgroundColor = "var(--button-bg-color)";
                    options.innerHTML = `<input type="text" class="search-input" oninput="filterOptions(\'${ids[i]}\', this.value)" placeholder="Search..." style="background-color: var(--button-bg-color) !important; width: 100%; padding: 8px; margin-bottom: 8px; border: 1px solid #ccc;">`;
                }
            }
        }
